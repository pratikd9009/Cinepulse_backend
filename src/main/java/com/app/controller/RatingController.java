...
public class RatingController {
    ...
    @GetMapping("/{ratingId}")
    public ResponseEntity<RatingDTO> getRatingById(@PathVariable("ratingId") Long ratingId) {
        RatingDTO ratingDTO = ratingService.getRatingById(ratingId);
        return new ResponseEntity<>(ratingDTO, HttpStatus.OK);
    }

    @GetMapping("/movie/{movieId}")
    public ResponseEntity<List<RatingDTO>> getRatingsByMovieId(@PathVariable("movieId") Long movieId) {
